...
import { PRESENTATIONS } from "./presentations-reuniones.mjs";

const __dirname = dirname(fileURLToPath(import.meta.url));
const AUDIO_DIR = resolve(__dirname, "../public/audio/reuniones");
const VOICE = "Eddy (Spanish (Mexico))";
const RATE = 135;
const FORCE = process.argv.includes("--force");

const LESSON_KEYS = {
  reunionesEfectivas: "reuniones-efectivas",
  prepararAgenda: "preparar-agenda",
  actasYSeguimiento: "actas-y-seguimiento",
  buenasPracticas: "buenas-practicas-reuniones",
};
...
for (const [key, json] of Object.entries(PRESENTATIONS)) {
  ...
  data.slides.forEach((slide, i) => {
    ...
    const audioUrl = `/audio/reuniones/${filename}`;
...
writeFileSync(resolve(__dirname, "./presentations-reuniones-with-audio.mjs"), moduleContent);
console.log(`\n✅ Audio reuniones listo (${Object.keys(LESSON_KEYS).length} lecciones)`);
