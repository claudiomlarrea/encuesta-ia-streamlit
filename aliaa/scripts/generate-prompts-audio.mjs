...
import { PRESENTATIONS } from "./presentations-prompts.mjs";

const __dirname = dirname(fileURLToPath(import.meta.url));
const AUDIO_DIR = resolve(__dirname, "../public/audio/prompts");
...
const LESSON_KEYS = {
  ...
  estructuraCTF: "estructura-ctf",
  tecnicas: "tecnicas",
  casosPracticos: "casos-practicos",
};
...
for (const [key, json] of Object.entries(PRESENTATIONS)) {
  ...
  data.slides.forEach((slide, i) => {
    ...
    const audioUrl = `/audio/prompts/${filename}`;
...
writeFileSync(resolve(__dirname, "./presentations-prompts-with-audio.mjs"), moduleContent);
console.log(`\n✅ Audio prompts listo (${Object.keys(LESSON_KEYS).length} lecciones)`);
