...
import { PRESENTATIONS } from "./presentations-ia.mjs";

const __dirname = dirname(fileURLToPath(import.meta.url));
const AUDIO_DIR = resolve(__dirname, "../public/audio/ia");
...
const LESSON_KEYS = {
  queEsIA: "que-es-ia",
  queEsToken: "que-es-token",
  queEsPrompt: "que-es-prompt",
};
...
for (const [key, json] of Object.entries(PRESENTATIONS)) {
  ...
  data.slides.forEach((slide, i) => {
    ...
    const audioUrl = `/audio/ia/${filename}`;
...
writeFileSync(resolve(__dirname, "./presentations-ia-with-audio.mjs"), moduleContent);
console.log(`\n✅ Audio IA listo (${Object.keys(LESSON_KEYS).length} lecciones)`);
