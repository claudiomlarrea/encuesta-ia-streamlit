...
import { PRESENTATIONS } from "./presentations-mineria.mjs";

const __dirname = dirname(fileURLToPath(import.meta.url));
const AUDIO_DIR = resolve(__dirname, "../public/audio/mineria");
const VOICE = "Eddy (Spanish (Mexico))";
const RATE = 135;
const FORCE = process.argv.includes("--force");

const LESSON_KEYS = {
  iaEnMineria: "ia-en-mineria",
  informesComunicacion: "informes-comunicacion",
  datosOperativos: "datos-operativos",
  seguridadAmbiente: "seguridad-ambiente",
};
...
for (const [key, json] of Object.entries(PRESENTATIONS)) {
  ...
  data.slides.forEach((slide, i) => {
    ...
    const audioUrl = `/audio/mineria/${filename}`;
...
writeFileSync(resolve(__dirname, "./presentations-mineria-with-audio.mjs"), moduleContent);
console.log(`\n✅ Audio minería listo (${Object.keys(LESSON_KEYS).length} lecciones)`);
