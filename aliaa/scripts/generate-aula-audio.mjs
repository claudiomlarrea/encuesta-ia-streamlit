...
import { PRESENTATIONS } from "./presentations-aula.mjs";

const __dirname = dirname(fileURLToPath(import.meta.url));
const AUDIO_DIR = resolve(__dirname, "../public/audio/aula");
const VOICE = "Eddy (Spanish (Mexico))";
const RATE = 135;
const FORCE = process.argv.includes("--force");

const LESSON_KEYS = {
  iaEnEducacion: "ia-en-educacion",
  promptPedagogico: "prompt-pedagogico",
  planificarAsignatura: "planificar-asignatura",
  rubricasYActividades: "rubricas-y-actividades",
  debatesYPersonalizacion: "debates-y-personalizacion",
  evaluacionYEtica: "evaluacion-y-etica",
};
...
for (const [key, json] of Object.entries(PRESENTATIONS)) {
  ...
  data.slides.forEach((slide, i) => {
    ...
    const audioUrl = `/audio/aula/${filename}`;
...
writeFileSync(resolve(__dirname, "./presentations-aula-with-audio.mjs"), moduleContent);
console.log(`\n✅ Audio aula listo (${Object.keys(LESSON_KEYS).length} lecciones)`);
